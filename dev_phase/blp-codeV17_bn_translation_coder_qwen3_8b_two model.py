...
translation_model_name = "md-nishat-008/TigerLLM-1B-it"
...
for i, row in tqdm(df.iterrows(), total=len(df)):
    ...
    promptt = f"Translate this from Bengali to English:\n{bengali_instruction}"
    inputs = translator_tokenizer(promptt, return_tensors="pt", padding=True, truncation=True)
    translated_tokens = translator_model.generate(**inputs, max_length=512)
    english_instruction = translator_tokenizer.decode(translated_tokens[0], skip_special_tokens=True)
